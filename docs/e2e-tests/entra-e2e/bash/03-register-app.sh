...
APP_ID="$(az ad app list --identifier-uri "$APP_IDENTIFIER" --query "[0].appId" -o tsv 2>/dev/null || true)"
if [[ -n "$APP_ID" ]]; then
...
if [[ -z "$APP_ID" ]]; then
    if APP_ID="$(az ad app create \
            ...
            --query appId -o tsv 2>/dev/null)" && [[ -n "$APP_ID" ]]; then
        az ad sp create --id "$APP_ID" >/dev/null
        echo "Created app registration via CLI: $APP_ID"
    else
        APP_ID=''
    fi
fi
...
Register the app manually in the Azure portal:
  ...
  3. -> Expose an API
       ...
       NOTE: copy this value -- it must match APP_IDENTIFIER in _config.sh.

EOF
    read -r -p "Paste the AppId (the GUID, NOT the api://... URI) and press <Enter>: " APP_ID
    APP_ID="${APP_ID// /}"
    if [[ -z "$APP_ID" ]]; then
        echo "ERROR: no AppId supplied; aborting." >&2
        exit 1
    fi
    echo "Using AppId: $APP_ID"

    if [[ "$APP_IDENTIFIER" != "api://${APP_ID}" ]]; then
        cat <<EOF >&2

WARNING: APP_IDENTIFIER in _config.sh is currently:
    $APP_IDENTIFIER
but the standard portal value for the app you just created is:
    api://${APP_ID}

Update _config.sh and re-source it before running step 04, otherwise the
caller-token resource won't match the server's ENTRA_AUDIENCE.
EOF
    fi
fi

save_state APP_ID "$APP_ID" TENANT_ID "$TENANT_ID"
...
read -r -p "Press <Enter> once the scope has been added to continue: " _
echo "Done. Next: ./04-run-server.sh"
