...
These checks cover the controls implemented under [release-readiness-check.md](./release-readiness-check.md) §4a: pipeline namespace enforcement, denied database/collection blocklists, and the `readOnly` profile flag. Each check restarts the server with the JSON shown, then exercises the named tool. Restore the default profile JSON between checks.
...
4. **`readOnly: true` denies write tools regardless of `allowedRoles`.** Use `'{"dev":{"uri":"mongodb://...","allowedRoles":["read","write","management"],"readOnly":true}}'`. Call `insert_documents`. Expect `isError: true` with `Tool tier 'write' is not allowed ... Profile is configured as readOnly.`. Then call `find_documents` and expect normal success.
5. **`readOnly: true` denies management tools.** With the same profile from step 4, call `drop_collection` (with proper confirmation). Expect `isError: true` with `Tool tier 'management' is not allowed ... Profile is configured as readOnly.`.
6. **Pipeline `$lookup.from` is gated by the collection allowlist.** Use `'{"dev":{"uri":"mongodb://...","allowedRoles":["read","write","management"],"allowedDatabases":["fleet"],"allowedCollections":{"fleet":["vehicles"]}}}'`. Call `aggregate` on `fleet.vehicles` with `pipeline=[{"$lookup":{"from":"audit_log","localField":"id","foreignField":"vid","as":"a"}}]`. Expect `isError: true` with `Aggregation stage $lookup references fleet.audit_log: Collection 'fleet.audit_log' is not allowed`.
7. **Pipeline `$unionWith` is gated by the denylist.** Use `'{"dev":{"uri":"mongodb://...","allowedRoles":["read","write","management"],"deniedCollections":{"fleet":["audit_log"]}}}'`. Call `aggregate` on `fleet.vehicles` with `pipeline=[{"$unionWith":"audit_log"}]`. Expect `isError: true` with `Aggregation stage $unionWith references fleet.audit_log: Collection 'fleet.audit_log' is denied`.
8. **Cross-database `$lookup` is gated.** Use `'{"dev":{"uri":"mongodb://...","allowedRoles":["read","write","management"],"allowedDatabases":["fleet"]}}'`. Call `aggregate` on `fleet.vehicles` with `pipeline=[{"$lookup":{"from":{"db":"secrets","coll":"passwords"},"as":"p"}}]`. Expect `isError: true` with `Aggregation stage $lookup references secrets.passwords: Database 'secrets' is not allowed`.
9. **`explain_operation` (`operation=aggregate`) is gated.** With the profile from step 6, call `explain_operation` with `operation="aggregate"` and `pipeline=[{"$lookup":{"from":"audit_log","as":"a"}}]`. Expect `isError: true` with the same `Aggregation stage $lookup` message.
