...
## Gap 7: Aggregation pipelines bypass the per-profile resource allowlist — CLOSED

**What's missing.** ~~Section 3's allowlist is enforced against the top-level `db_name` / `collection_name` tool inputs in `withDbGuard`. It is not applied to namespace references *inside* an aggregation pipeline.~~ Closed in [release-readiness-check.md](./release-readiness-check.md) §4a by `assertPipelineNamespacesAllowed` ([src/tools/utils/pipelineNamespaces.ts](../src/tools/utils/pipelineNamespaces.ts)), wired into both `aggregate` and `explain_operation` (`operation=aggregate`). The walker recursively visits `$lookup.from` (string and `{db, coll}` form, plus nested `pipeline`), `$unionWith` (string and object form), `$graphLookup.from`, `$merge.into`, `$out`, and `$facet` sub-pipelines, and applies the same `assertResourceAllowed` check used by the top-level guard. Tests: [test/tools/pipelineNamespaces.test.ts](../test/tools/pipelineNamespaces.test.ts) plus 5 wiring tests in [test/tools/registeredTools.test.ts](../test/tools/registeredTools.test.ts).

**Original analysis (kept for context).** A caller scoped to `fleet.vehicles` could previously reach other collections via `$lookup.from`, `$lookup.pipeline` (recursive), `$unionWith`, `$graphLookup.from`, or `$facet` sub-pipelines. With `ALLOW_AGGREGATE_WRITE_STAGES=true` the same gap previously applied to `$merge.into` / `$out` writes. Both read- and write-side cross-namespace paths are now gated.
...
4. ~~**Required.** Land **Pipeline namespace enforcement** (Gap 7) — self-contained walker plus wiring for `aggregate` and `explain_operation`. Without it, the section 3 allowlist over-promises.~~ **DONE** — see Gap 7.
