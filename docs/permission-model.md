...
|---|---|
| Where | [src/security/connectionProfiles.ts](../src/security/connectionProfiles.ts) |
| Inputs | `allowedDatabases`, `allowedCollections` on the resolved profile, plus tool-input `db_name`, `collection_name`, `new_collection_name` |
| Default | omitted = unrestricted; `[]` = **explicit deny-all**; `[...]` = narrow to listed entries (uniform with `allowedRoles`) |
| Effect | Per-profile allowlist of databases and per-database collections |
| On deny | "Database 'secrets' is not allowed for connection profile 'dev'. Allowed databases: fleet." (or `(none)` for `[]`) |

For `rename_collection`, the source and destination collection names are both checked.

`list_databases` additionally filters its **response** by the same scope ([src/tools/database-tools.ts](../src/tools/database-tools.ts)) so callers cannot enumerate databases or collections outside the allowlist.

**Known limitation:** this gate inspects the top-level `db_name` / `collection_name` only. It does not walk aggregation pipelines, so `$lookup.from`, `$unionWith`, `$graphLookup.from`, and `$facet` sub-pipelines can still reach other namespaces. Cross-namespace **writes** via `$out` / `$merge` are blocked by default through `assertAggregatePipelineIsReadOnly` (next layer). See [release-readiness-check.md](./release-readiness-check.md) §3 and [docs/gaps-on-rbac.md](./gaps-on-rbac.md) Gap 7.
...
- **Pipeline namespace bypass.** Gate [9] does not walk aggregation pipelines.
