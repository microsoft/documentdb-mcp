...
export function registerCollectionResources(server: McpServer) {
  ...
  const collectionVarTemplate = (suffix: string) => new ResourceTemplate(`collection/{db}/{collection}/${suffix}`, {
  ...
  server.registerResource(
    ...
    async (_uri, vars) => {
      ...
      return {
        contents: [{ uri: _uri.toString(), mimeType: 'application/json', text: JSON.stringify(indexes, null, 2) }]
      } as any;
    }
  );
  ...
  server.registerResource(
    ...
    { title: 'Collection Schema', description: 'Schema information (inferred) for a collection' },
    async (_uri, vars) => {
      const { client } = getDocumentDBContext();
      const dbName = Array.isArray(vars.db) ? vars.db[0] : vars.db;
      const collName = Array.isArray(vars.collection) ? vars.collection[0] : vars.collection;
      ...
      return { contents: [{ uri: _uri.toString(), mimeType: 'application/json', text: JSON.stringify(schema, null, 2) }] } as any;
    }
  );
  ...
  server.registerResource(
    ...
    { title: 'Collection Stats', description: 'Performance and size statistics for a collection' },
    async (_uri, vars) => {
      const { client } = getDocumentDBContext();
      const dbName = Array.isArray(vars.db) ? vars.db[0] : vars.db;
      const collName = Array.isArray(vars.collection) ? vars.collection[0] : vars.collection;
      ...
      return { contents: [{ uri: _uri.toString(), mimeType: 'application/json', text: JSON.stringify(stats, null, 2) }] } as any;
    }
  );
  ...
  server.registerResource(
    ...
    { title: 'Collection Validation', description: 'Validation rules (JSON Schema) for a collection' },
    async (_uri, vars) => {
      const { client } = getDocumentDBContext();
      const dbName = Array.isArray(vars.db) ? vars.db[0] : vars.db;
      const collName = Array.isArray(vars.collection) ? vars.collection[0] : vars.collection;
      ...
      return { contents: [{ uri: _uri.toString(), mimeType: 'application/json', text: JSON.stringify(validation, null, 2) }] } as any;
    }
  );
  ...
  server.registerResource(
    ...
    async (_uri, vars) => {
      ...
      return { contents: [{ uri: _uri.toString(), mimeType: 'application/json', text: JSON.stringify(names, null, 2) }] } as any;
    }
  );
}
