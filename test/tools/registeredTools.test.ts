...
describe('registered DocumentDB tools — per-profile capability tier restrictions', () => {
    ...
    it('rejects a write tool when allowWriteTools=false (overrides global flag)', async () => {
        const profiles =
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toMatch(/Write tools are disabled for connection profile 'dev'/);
        expect(collection.updateOne).not.toHaveBeenCalled();
        expect(collection.updateMany).not.toHaveBeenCalled();
    });

    it('rejects a management tool when allowManagementTools=false', async () => {
        const profiles =
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toMatch(/Management tools are disabled for connection profile 'dev'/);
    });

    it('still allows read tools when only write+management are profile-disabled', async () => {
        const profiles =
            '{"dev":{"uri":"mongodb://fake","allowWriteTools":false,"allowManagementTools":false}}';
        const { tools, collection } = await setupRegisteredTools({ CONNECTION_PROFILES: profiles });

        const response = await tools.count_documents.handler({
            connection_profile: 'dev',
            db_name: 'fleet',
            collection_name: 'vehicles',
            query: {},
    ...
    it('allowWriteTools=false denies even when allowedRoles permits write', async () => {
        const profiles =
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toMatch(/Write tools are disabled/);
